...
def calcular_fitness(solution: List[Hotel], max_hotel_stars: int, budget: float, alpha=1.0, beta=1.0, gamma=1.0) -> float:
    """
    ...
    # Normalización
    stars_norm   = total_stars / (len(solution) * max_hotel_stars) if max_hotel_stars > 0 else 0
    cost_norm    = total_cost  / budget if budget > 0 else 1
    changes_norm = changes / (len(solution)-1) if len(solution) > 1 else 0

    # Suma ponderada
    fitness = alpha * stars_norm - beta * cost_norm - gamma * changes_norm
    return fitness
