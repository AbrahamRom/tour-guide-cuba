...
import ir_datasets  # Comentado porque no se puede instalar por el momento
...
def simulate_search_query(query, correct=True, top_k=10):
    corrected = (
        suggest_query(query, [doc["title"] for doc in retriever.documents])
        if correct
        else query
    )
...
def evaluate_searcher_with_dataset(top_k=10, correct=True, max_queries=30):
    ...
    for q in queries[:n_eval]:
        ...
        retrieved = [
            r[0]["doc_id"]
            for r in retriever.search(res["corrected_query"], top_k=top_k)
        ]
